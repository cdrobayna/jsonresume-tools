...
Don't have a JSON Resume yet? See [start with an example](/guide/start-with-an-example) instead.
...
| Generate a backend/platform/whatever-role variant from **one** base resume     | [`jrt`](/reference/tailor) (jsonresume-tailor)       |
...
Generates role-tailored variants from a single annotated base resume: tag `work`/`skills`/etc.
entries under `meta.tailor`, declare a variant per target role, and `jrt build <variant>` emits a
filtered, canonical JSON Resume, with no trace of `tailor` left in it. Use it once you're tired
of manually maintaining separate `resume.backend.json` / `resume.devops.json` copies by hand.
...
language, and `jrx check` runs every validator across base resumes and the matrix in one pass.
See the [full workflow tutorial](/guide/full-workflow) for a worked example, end to end.
