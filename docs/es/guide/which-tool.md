...
| Generar una variante backend/platform/lo-que-sea desde **un** único master resume | [`jrt`](/es/reference/tailor) — jsonresume-tailor |
...
inversa, URLs/email válidos, marcadores `TODO`/`FIXME` sin borrar, y validación estricta de schema
opcional. Úsalo en cada JSON Resume que tengas, incluso si mantienes un solo archivo en un solo
idioma.
...
Genera variantes por rol desde un único master resume anotado: etiqueta entradas de
...
por idioma, y `jrx check` corre todos los validadores contra masters y matriz en una sola pasada.
Consulta el [tutorial de flujo completo](/es/guide/full-workflow) para un ejemplo completo, de
punta a punta.
